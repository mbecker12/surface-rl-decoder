...
class EpisodeBuffer:
    def __init__(self, args) -> None:
        ...
        self.discounts = np.logspace(
            0, self.max_steps+1, num=self.max_steps+1, base=self.gamma, endpoint=False, dtype=np.float128)
        self.tau_discounts = np.logspace(
            0, self.max_steps+1, num=self.max_steps+1, base=self.tau*self.gamma, endpoint=False, dtype=np.float128)

        self.clear()

    def clear(self):
        self.states_mem = np.empty(
            shape=np.concatenate(((self.max_episodes, self.max_steps), (self.stack_depth, self.syndrome_size, self.syndrome_size))), dtype=np.uint8
        )
        # self.states_mem[:] = np.nan # TODO why?

        self.actions_mem = np.empty(shape=(self.max_episodes, self.max_steps, 3), dtype=np.uint8)
        # self.actions_mem[:] = np.nan

        self.rewards_mem = np.empty(shape=(self.max_episodes, self.max_steps), dtype=np.float32)
        self.rewards_mem[:] = np.nan

        # gae = Generalized advantage estimation
        self.gaes_mem = np.empty(shape=(self.max_episodes, self.max_steps), dtype=np.float32)
        self.gaes_mem[:] = np.nan

        # log probability of action
        self.logpas_mem = np.empty(shape=(self.max_episodes, self.max_steps), dtype=np.float32)
        self.logpas_mem[:] = np.nan

        self.values_mem = np.empty(shape=(self.max_episodes, self.max_steps, 1), dtype=np.float32)
    ...
    def fill(self, env_set: MultiprocessEnv, combined_model):
        ...
        worker_rewards = np.zeros(shape=(self.num_workers, self.max_steps), dtype=np.float32)
        worker_exploratory = np.zeros(shape=(self.num_workers, self.max_steps), dtype=np.bool)
        worker_steps = np.zeros(shape=(self.num_workers), dtype=np.uint16)
        worker_seconds = np.array([time(),] * self.num_workers, dtype=np.float64)
        
        buffer_full = False
        while not buffer_full and len(self.episode_steps[self.episode_steps > 0]) < self.max_episodes * 0.75:
            with torch.no_grad():
                # TODO parse states to torch
                # TODO note that policy and value models are separate
                actions, logpas, are_exploratory, values = combined_model.np_pass(states)
            ...
            if terminals.sum():
                ...
                if truncated.sum():
                    ...
                    with torch.no_grad():
                        _, tmp_next_values = combined_model(
                            next_states[idx_truncated]
                        )
            ...
            if terminals.sum():
                ...
                for w_idx in idx_terminals:
                    ...
                    self.episode_exploration[e_idx] = worker_exploratory[w_idx, :steps].mean()
                    ...
                    ep_returns = np.array(
                        [np.sum(ep_discounts[:steps+1 - t] * ep_rewards[t:]) for t in range(steps)]
                    )
                    ...
                    with torch.no_grad():
                        ...
                        gaes = np.array(
                            [np.sum(self.tau_discounts[:steps-t] * deltas[t:]) for t in range(steps)]
                        )
        ...
        self.states_mem = [row[:ep_steps[i]] for i, row in enumerate(self.states_mem[ep_idxs])]
        self.states_mem = np.concatenate(self.states_mem)

        self.actions_mem = [row[:ep_steps[i]] for i, row in enumerate(self.actions_mem[ep_idxs])]
        self.actions_mem = np.concatenate(self.actions_mem)

        self.rewards_mem = [row[:ep_steps[i]] for i, row in enumerate(self.rewards_mem[ep_idxs])]
        self.rewards_mem = torch.tensor(
            np.concatenate(self.rewards_mem),
            device=combined_model.device
        )

        self.gaes_mem = [row[:ep_steps[i]] for i, row in enumerate(self.gaes_mem[ep_idxs])]
        self.gaes_mem = torch.tensor(
            np.concatenate(self.gaes_mem),
            device=combined_model.device
        )

        self.logpas_mem = [row[:ep_steps[i]] for i, row in enumerate(self.logpas_mem[ep_idxs])]
        self.logpas_mem = torch.tensor(
            np.concatenate(self.logpas_mem),
            device=combined_model.device
        )

        self.values_mem = [row[:ep_steps[i]] for i, row in enumerate(self.values_mem[ep_idxs])]

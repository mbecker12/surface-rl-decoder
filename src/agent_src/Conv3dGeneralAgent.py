...
class Conv3dGeneralAgent(nn.Module):
    ...
    """
    Description:
        Fourth iteration of an agent. Consists of multiple 3D convolutional layers.
        Splits the input into x,z and both errors which it then proceeds to feed each part into a neural network
        before adding them together and feeding them together into 2 linear layers from which only the final (with regards to time) output is used.
    ...
    def __init__(self, config):
        ...
        self.almost_final_layer = nn.Linear((self.size+1)*(self.size+1)*self.output_channels4 , self.neurons_lin_layer)
        self.final_layer = nn.Linear(self.neurons_lin_layer,self.nr_actions_per_qubit*(self.size)*(self.size)+1)
    ...
    def forward(self, state):
        ...
        if self.split_input_toggle:
            ...
            both = both.view(-1, self.input_channels, self.stack_depth, (self.size+1), (self.size+1)) #convolve both
            ...
            complete = complete.view(-1, self.stack_depth,  (self.size+1)*(self.size+1)*self.output_channels4) #make sure the dimensions are in order
        else:
            both = state.view(-1, self.input_channels, self.stack_depth, (self.size+1), (self.size+1)) #convolve both
            both = F.relu(self.input_conv_layerBoth(both))
            both = F.relu(self.nd_conv_layerBoth(both))
            both = self.rd_conv_layerBoth(both)
            both = self.comp_conv_layerBoth(both)
 
            complete = both.view(-1, self.stack_depth,  (self.size+1)*(self.size+1)*self.output_channels4) #make sure the dimensions are in order



        complete = self.almost_final_layer(complete)
        complete = complete.view( -1, self.stack_depth, self.nr_actions_per_qubit*(self.size)*(self.size)+1) #shift the dimension so that
    ...
    def interface(self, state): #helper function to split up the input

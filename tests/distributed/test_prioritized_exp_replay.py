...
def test_compute_priorities():
    ...
    rewards = np.random.randint(0, 2, size=(num_environments, buffer_size-1), dtype=bool)
    qvalues = np.random.random_sample(
        size=(
            num_environments,
            buffer_size-1,
            3 * system_size * system_size + 1
        )
    )
    gamma = 1.0
    priorities = compute_priorities(actions, rewards, qvalues, gamma, system_size)
    assert priorities.shape == (num_environments, buffer_size-1, )
...
def test_deterministic_priorities():
    ...
    assert qvalues.shape == (num_environments, buffer_size-1, 3 * system_size ** 2 + 1)
    ...
    priorities = compute_priorities(actions, rewards, qvalues, gamma, system_size)
    assert np.all(priorities == expected_priorities), f"{priorities=}, {expected_priorities=}"
...
def test_sampling():
    ...
    states = np.random.randint(0, 2, size=(num_environments, buffer_size, stack_depth, state_size, state_size), dtype=np.uint8)
    next_states = np.random.randint(0, 2, size=(num_environments, buffer_size, stack_depth, state_size, state_size), dtype=np.uint8)
    terminals = np.random.randint(0, 2, size=(num_environments, buffer_size), dtype=bool)

    transitions = np.asarray(
        [
            Transition(
                states[i][buf], actions[i][buf], rewards[i][buf], next_states[i][buf], terminals[i][buf]
            )
